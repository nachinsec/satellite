...
use tauri::Emitter;
use crate::minecraft_api::{MinecraftVersion, LauncherError};
use crate::config::LauncherConfig;
use reqwest::Client;

fn main() {
    tauri::Builder::default()
    .invoke_handler(tauri::generate_handler![start_launcher, get_versions, get_config])
...
async fn start_launcher_internal(window: tauri::Window, version: String) -> Result<(), LauncherError> {
    let config = LauncherConfig::load().map_err(|e| LauncherError::ConfigValidation { field: "config".to_string(), message: e.to_string() })?;
    ...
    window.emit("log", format!("Downloading version JSON: {}", selected_version.id)).ok();
    let version_json_str = minecraft_api::download_version_json(&client, &selected_version.url).await?;
    let version_json = minecraft_api::parse_version_json(&version_json_str)?;

    window.emit("log", "Downloading main JAR...").ok();
    let jar_path = format!("{}/versions/{}/{}.jar", base_dir, selected_version.id, selected_version.id);
    let jar_url = &version_json.downloads.client.url;
    minecraft_api::download_file(&client, jar_url, &jar_path).await?;
    window.emit("log", format!("Jar downloaded to: {}", jar_path)).ok();
                    ...
                    if std::path::Path::new(&lib_path).exists() {
                        if let Ok(mut file) = File::open(&lib_path) {
                            ...
                            if &actual_sha1 == expected_sha1 {
                                ...
                                window.emit("log", format!("Hash mismatch, re-downloading: {}", lib_path)).ok();
                            }
                        }
                    }
    ...
    minecraft_api::download_file(&client, &version_json.asset_index.url, &assets_index_path).await?;
    ...
    let _output = command.arg("-cp").arg(&classpath_str).arg(&version_json.main_class)
        .arg("--username").arg(&config.player_name)
        .arg("--version").arg(&selected_version.id)
        .arg("--gameDir").arg(base_dir)
        .arg("--assetsDir").arg(format!("{}/assets", base_dir))
        .arg("--assetIndex").arg(&version_json.asset_index.id)
        .arg("--uuid").arg(&config.player_uuid.unwrap_or_default())
        .arg("--accessToken").arg("N/A")
        .arg("--userType").arg("legacy")
        .arg("--versionType").arg(&selected_version.r#type)
        .spawn()
        .map_err(|e| LauncherError::MinecraftLaunchError { error: e.to_string() })?;
    
    window.emit("log", format!("Java command launched, check if Minecraft window opens.")).unwrap();
    Ok(())
}
