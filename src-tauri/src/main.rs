...
fn main() {
    ...
    .invoke_handler(tauri::generate_handler![start_launcher, get_versions]) // start_launcher ahora espera version_id como parámetro
...
#[tauri::command]
async fn start_launcher(window: tauri::Window, version: String) {
    ...
    let selected_version = match selected_version {
        Some(v) => v,
        None => {
            window.emit("log", format!("Version not found: {}", version)).ok();
            return;
        }
    };

    window.emit("log", format!("Downloading version JSON: {}", selected_version.id)).ok();
    let version_json_str = minecraft_api::download_version_json(&client, &selected_version.url).await.unwrap();
...
// Solo agrega los jars requeridos por la versión seleccionada
for lib in &version_json.libraries {
    if let Some(downloads) = &lib.downloads {
        if let Some(artifact) = &downloads.artifact {
            if let Some(lib_path_rel) = &artifact.path {
...
let classpath_str = classpath.join(";");
    let _output = Command::new("java")
        ...
        .spawn()
        .expect("Minecraft launch failed");
